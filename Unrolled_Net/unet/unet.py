...
class UNetModel(nn.Module):
    ...
    def unnorm(
        self, x: th.Tensor, mean: th.Tensor, std: th.Tensor
    ) -> th.Tensor:
        return x * std + mean
    ...
    def forward(self, x, timesteps=None, y=None):
        ...
        hs = []
        if self.use_time_emb:
            # emb = self.time_embed(timestep_embedding(timesteps.to(dtype=x.dtype), self.temb_channel, max_period=self.max_freq))
            emb = self.time_embed(timestep_embedding(timesteps.to(dtype=x.dtype), self.model_channels))
            if self.num_classes is not None:
